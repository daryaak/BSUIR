...
void merge_sorted(char *filename)
{
    ...
    while (count < amount_of_recs) 
    {
        ...
        for (int i = 0; i < sorted_in_buffer; i++) 
        {
            ...
            if (tmp_val.time_mark < min_data.time_mark) 
            {
                min_data = tmp_val;
                min = i;
            }
        }
        fwrite(&min_data, size_of_record, 1, f);
        ++inds[min];
        ++count;
...
void *func(void *arg)
{
    ...
    while (true)
    {
        ...
        qsort(&(((index_s *) file_map)[thread_num * size_of_block / size_of_record]), size_of_block / size_of_record, size_of_record, compare);
        int cur_block;
        while (true) 
        {
            ...
            qsort(&(((index_s *) file_map)[cur_block* size_of_block/ size_of_record]), size_of_block / size_of_record, size_of_record, compare);
        }
        pthread_barrier_wait(&bar);
...
int main(int argc, char *argv[]) 
{
    if (argc != 5) 
    {
        fprintf(stderr, "HOW TO USE: %s MEMSIZE BLOCKS THREADS FILENAME\n", argv[0]);
        return EXIT_FAILURE;
    }
    ...
    if (size % pagesize) 
    {
        fprintf(stderr, "MEMSIZE SHOULD BE DIVISIBLE BY PAGESIZE\n");
        return EXIT_FAILURE;
    }

    if ((amount_of_blocks & (amount_of_blocks - 1))) 
    {
        fprintf(stderr, "BLOCKS SHOULD BE POWER OF 2\n");
        return EXIT_FAILURE;
    }

    if (amount_of_blocks <= amount_of_threads) 
    {
        fprintf(stderr, "AMOUNT OF BLOCKS SHOULD BE MORE THAN THREADS\n");
        return EXIT_FAILURE;
    }
    ...
    {
        fprintf(stderr, "AMOUNT OF THREADS SHOULD BE BETWEEN %d AND %d\n", core, N);
        return -1;
    }
    ...
    printf("\n");
    printf("%s SORTED AND SAVED TO %s\n\n", filename, SORTED_FILENAME);
